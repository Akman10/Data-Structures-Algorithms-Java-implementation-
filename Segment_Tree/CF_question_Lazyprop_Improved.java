...
public class ques4
{
	...
	void solve() throws Exception
	{
		...
		for(int i=0;i<q;i++) 
		{
			...
			if(type==1)
			{
				int in=first(0,x-1,val);
				if(in!=-1)
				update(1,0,n-1, in, x-1, val);
			}
			else
			{
				ans=0;
				money=val;
				get2(1,0,n-1,x-1,n-1);
					out.println(ans);
			}
		}
	}
	
	void get2(int v,int tl,int tr,int l, int r)
	{
		if(l>r||l>tr || r<tl || mn[v]>money)return;
		if(tl>=l && tr<=r && t[v]<=money)
		{
			money-=t[v];
			ans+=tr-tl+1;
			return;
		...
		int tm = (tl + tr) >> 1;
		get2(v << 1, tl, tm, l,r);
		get2(v << 1 | 1, tm + 1, tr,l,r);
	}
	
	int first(int l,int r,long val)
	{
		int f=0; int index=-1;
		while(l<=r)
		{
			int m=(l+r)/2;
			ans=0; get(1,0,n-1,m,m);
			if(ans<val)
				{f=1;index=m;}
			if(ans<val)
			{
